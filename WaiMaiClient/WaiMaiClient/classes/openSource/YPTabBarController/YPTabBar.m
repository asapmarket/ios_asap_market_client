...
- (void)setItems:(NSArray *)items {
    ...
    for (YPTabItem *item in self.items) {
        ...
        item.layer.borderColor = [UIColor whiteColor].CGColor;//self.itemBorderColor.CGColor;
        if (self.itemBgColor) {
...
- (void)updateItemsFrame {
    ...
    if (self.scrollView) {
        ...
        for (NSInteger index = 0; index < self.items.count; index++) {
            ...
            item.frame = CGRectMake( x + (self.itemSeparatorWidth * index), 0, width, self.frame.size.height);
            item.index = index;
            x += width;
        ...
        CGFloat x = self.leftAndRightSpacing;
        self.itemWidth = (self.frame.size.width - self.leftAndRightSpacing * 2 - self.itemSeparatorWidth * (self.items.count -1)) / (self.items.count + (self.specialItem != nil ? 1 : 0));
        // 四舍五入，取整，防止字体模糊
        self.itemWidth = floorf(self.itemWidth + 0.5f);

        for (NSInteger index = 0; index < self.items.count; index++) {
            YPTabItem *item = self.items[index];
//////////// modified 2017-10-12 10:50:05 pop
//            if (index == self.items.count - 1) {
//                self.itemWidth = self.frame.size.width - x;
//            }
            item.frame = CGRectMake(x + (index * self.itemSeparatorWidth), 0, self.itemWidth, self.frame.size.height);
            item.index = index;
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (_selectedItemIndex >= 0) {
        ...
        oldSelectedItem.backgroundColor = self.itemBgColor;
    ...
    if (self.itemBgColor) {
        newSelectedItem.backgroundColor = self.itemSelectedBgColor;
    }
...
- (void)setItemCornerRadius:(CGFloat)itemCornerRadius{
    ...
    [self.items makeObjectsPerformSelector:@selector(setRadius:) withObject:@(itemCornerRadius)];
    
}

- (void)setItemBgColor:(UIColor *)itemBgColor{
    _itemBgColor = itemBgColor;
    [self.items makeObjectsPerformSelector:@selector(setBackgroundColor:) withObject:itemBgColor];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.isDragging || scrollView.isDecelerating) {
        ...
        if (self.itemColorChangeFollowContentScroll) {
            ...
            if (self.itemBgColor) {
                [self.itemBgColor getRed:&normalRed green:&normalGreen blue:&normalBlue alpha:nil];
                [self.itemSelectedBgColor getRed:&selectedRed green:&selectedGreen blue:&selectedBlue alpha:nil];
                
                redDiff = selectedRed - normalRed;
                greenDiff = selectedGreen - normalGreen;
                blueDiff = selectedBlue - normalBlue;
                leftItem.backgroundColor= [UIColor colorWithRed:leftScale * redDiff + normalRed
                                                          green:leftScale * greenDiff + normalGreen
                                                           blue:leftScale * blueDiff + normalBlue
                                                          alpha:1];
                rightItem.backgroundColor = [UIColor colorWithRed:rightScale * redDiff + normalRed
                                                            green:rightScale * greenDiff + normalGreen
                                                             blue:rightScale * blueDiff + normalBlue
                                                            alpha:1];
    ...
    if ([self.delegate respondsToSelector:@selector(yp_tabBar:didScroll:)]) {
        [self.delegate yp_tabBar:self didScroll:scrollView];
    }
}
