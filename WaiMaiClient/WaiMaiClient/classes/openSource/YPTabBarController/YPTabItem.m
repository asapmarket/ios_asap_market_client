...
- (void)updateBadge {
    if (self.badgeStyle == YPTabItemBadgeStyleNumber) {
        if ([self.badge integerValue] == 0 || self.badge == nil) {
            self.badgeButton.hidden = YES;
        } else {
//            NSInteger *badgeNum = [self.badge integerValue];
//            if (self.badge > 99) {
//                badgeStr = @"99+";
...
//            }
            ...
            // 计算badgeStr的size
            CGSize size = [self.badge boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)
            ...
            // 设置badgeButton的frame
            self.badgeButton.frame = CGRectMake(self.bounds.size.width/2 + self.numberBadgeCenterMarginRight,
                                                self.numberBadgeMarginTop,
                                                width,
                                                height);
            self.badgeButton.layer.cornerRadius = self.badgeButton.bounds.size.height / 2;
            [self.badgeButton setTitle:self.badge forState:UIControlStateNormal];
